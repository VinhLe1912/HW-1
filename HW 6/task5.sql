...
#1
SELECT first_name, surname, bill_date, COUNT(bill_no) AS number_of_bills
FROM restBill A1
INNER JOIN restStaff A2 
ON A1.waiter_no = A2.staff_no
GROUP BY first_name, surname, bill_date
HAVING COUNT(bill_no) >= 2;

#2
SELECT room_name, COUNT(no_of_seats) AS number_of_table
FROM restRest_table
GROUP BY room_name
HAVING COUNT(no_of_seats) >2; 

#3
SELECT room_name, COUNT(bill_total) AS amount_of_bill
FROM restBill C1
INNER JOIN restRest_table C2
ON C1.table_no = C2.table_no
GROUP BY room_name;

#4
SELECT D3.first_name, D3.surname, COUNT(bill_no) AS total_bill_amount
FROM restStaff D1
INNER JOIN restBill D2
ON D1.staff_no = D2.waiter_no
INNER JOIN restStaff D3
ON D3.staff_no = D1.headwaiter
...
#6
SELECT first_name, surname, COUNT(bill_no) AS number_of_bills
FROM restBill A1
INNER JOIN restStaff A2 
ON A1.waiter_no = A2.staff_no
GROUP BY first_name, surname
