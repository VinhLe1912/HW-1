...
(SELECT staff_no FROM restStaff WHERE headwaiter IN 
...
(SELECT room_name FROM restRest_table WHERE table_no IN
(SELECT table_no FROM restBill WHERE bill_date = "160111" AND cust_name LIKE "Nerida %" )));
...
FROM restStaff D1
WHERE NOT EXISTS
(SELECT * FROM restBill D2 INNER JOIN restStaff D3 ON D2.waiter_no = D3.staff_no 
 WHERE D1.staff_no = D2.waiter_no) AND headwaiter IS NOT NULL;
 
#5
SELECT F1.cust_name, F4.first_name, F4.surname, F3.room_name
FROM restBill F1
INNER JOIN restStaff F2
ON F1.waiter_no = F2.staff_no
INNER JOIN restRest_table F3
ON F1.table_no = F3.table_no
INNER JOIN restStaff F4
ON F2.headwaiter = F4.staff_no
WHERE bill_total IN (SELECT MAX(bill_total) FROM restBill)
